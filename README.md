...
3. Attaches news to movements that fall near their date.
4. Ingestion (talking to `yfinance`) and consumption (serving requests) are separate concerns:
   - `POST /api/ingest/{ticker}` — fetch a ticker's data now and persist it. This is also how a ticker starts being tracked.
   ...
   - `GET /api/tickers/{ticker}` — full stock + news data for a ticker, filterable by date range and move threshold. **Reads only from Postgres**, never calls `yfinance`.
   - `POST /api/chat` — ask questions about a ticker's movements; grounds Claude's answer in the same DB-only data. Also never calls `yfinance` directly.
